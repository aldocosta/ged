...
namespace GedOff.Controllers
{
    public class PastaDeptoController : Controller
    {
        ...
        public static string Mapa { get; set; }
        /// <summary>
        ...
        public ActionResult GerenciarPastasDaPasta()
        {
            ...
            //if (Session["iddepto"] != null)
            //{
            //    id = int.Parse(Session["iddepto"].ToString());
            //}
            //else
            //{
            ...
            //var pastas = dalpasta.RetornarLista().Where(p => p.CodigoPai == idpasta);

            //retornando pastas
            BLLPasta.RetornarPastasDepto(pasta.CodigoDepto).Where(p => p.CodigoPai == idpasta).ToList().ForEach(delegate(Pasta p)

              //dalpasta.RetornarLista().Where(p => p.CodigoPai == idpasta).ToList().ForEach(delegate(Pasta p)
             {
            ...
            //dalprocesso.RetornarLista().Where(p => p.IdPai == idpasta).ToList().ForEach(delegate(Processos p)
            {
                pastas.Add(new PastaProcesso()
                {
                    Codigo = p.Id,
